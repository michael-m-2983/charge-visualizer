...
const fragmentShaderSource = `
    ...
    uniform float u_targetPotential;
    ...
    void main() {
        ...
        float isEquipotential = step(u_targetPotential - u_distanceThreshold, potential) *
                                step(potential, u_targetPotential + u_distanceThreshold);

        if(isEquipotential > 0.0 && u_showEquipotential == 1) gl_FragColor = vec4(1,1,1,1);
    }
`;
...
const render = () => {
    ...
    const u_targetPotential = gl.getUniformLocation(shaderProgram, "u_targetPotential");
    gl.uniform1f(u_targetPotential, constants.target_potential);

    const u_distanceThreshold = gl.getUniformLocation(shaderProgram, "u_distanceThreshold");
    gl.uniform1f(u_distanceThreshold, constants.distance_threshold);
...
let charges = [
    {x: canvas.width / 4, y: canvas.height / 2, q: 10}, 
    {x: 3 * canvas.width / 4, y: canvas.height / 2, q: 10}
];
let point_counter = 1;
...
const setupGUI = () => {
    ...
    equipotentialFolder.add(constants, "target_potential", -10, 10).name("Target Potential").onChange(render);
    equipotentialFolder.add(constants, "distance_threshold", 0.01, 0.1).name("Distance Threshold").onChange(render);
    ...
    const addPointController = (point) => {
        const pointFolder = pointsFolder.addFolder(`Point ${point_counter++}`);
    ...
    gui.add({ add: () => {
        const newPoint = { x: canvas.width / 2, y: canvas.height / 2, q: 10 };
